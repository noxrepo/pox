...
"""
This component hides wire format details of DNS communication from
other modules.  Higher layer modules can answer DNS lookups and
process DNS answers without understanding DNS packet formats.
Although, additional low level details are exposed in the events
raised by dns_helper.
...
# The following patch is a workaround for a bug in (sliver) Open
# vSwitch version 1.11.90.
#
# --- a/pox/openflow/libopenflow_01.py
# +++ b/pox/openflow/libopenflow_01.py
# @@ -3530,7 +3530,7 @@ class ofp_packet_out (ofp_header):
#    def __init__ (self, **kw):
#      ofp_header.__init__(self)
#      self._buffer_id = NO_BUFFER
# -    self.in_port = OFPP_NONE
# +    self.in_port = OFPP_LOCAL
#      self.actions = []
#      self._data = b''
...
class DNSLookup (Event):
  def __init__ (self, rr):
    ...
    if t is not None:
      ...
      self.qtype_name = "OTHER"
    # event handlers should set only one of the following variables:
    self.rr_answers = []
    self.simple_answers = []
    # rr_answers is a list of full "rr"s, while
    # simple_answers is a list of IP addresses for A lookups, fully
    # qualified domain names for PTR lookups, etc.
...
class DNSHelper (EventMixin):
  ...
  def _send_response (self, event, answers):
    ...
    if q_udp.dstport == pkt_dns.MDNS_PORT:
  ...
  def _handle_ConnectionUp (self, event):
    if self._install_flow:
      ...
      install_one_flow(event, pkt_dns.SERVER_PORT)
      install_one_flow(event, pkt_dns.MDNS_PORT)
  ...
  def _handle_PacketIn (self, event):
    ...
    for q in p.questions:
      ...
      ev = DNSLookup(q)
      self.raiseEvent(ev)
      if ev.simple_answers and ev.rr_answers:
        log.error('simple_answers and rr_answers are mutually exclusive.')
        ev.simple_answers = []
      ...
      for rrdata in ev.simple_answers:
        ...
        rr = pkt_dns.rr(q.name, q.qtype, q.qclass, ttl, length, rrdata)
        answers.append(rr)
    if answers:
